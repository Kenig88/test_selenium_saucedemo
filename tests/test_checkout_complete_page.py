...
class TestCheckoutCompletePage:
    ...
    def test_user_can_see_checkout_complete_page_and_return_to_products(
        self, opened_checkout_complete_page, products_page
    ):
        assert (
            opened_checkout_complete_page.get_complete_header_text()
            == CheckoutCompleteMessages.HEADER
        )
        assert (
            opened_checkout_complete_page.get_complete_text()
            == CheckoutCompleteMessages.TEXT
        )
